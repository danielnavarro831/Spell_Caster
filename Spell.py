            ...
            for stat in range(len(Player.Stats)):
                if Player.Stats[stat]["Name"] == self.duration_spells[spell] and Player.Stats[stat]["Active"] == True:
                    if Player.Stats[stat]["Name"] == Player.Regen["Name"]:
                        ...
                        Scene.log.append(Scene.make_line(4, "Menu", {"{Name}": Player.Name["Value"], "{Amount}": amount, "{Stat}": Player.HP["Name"]}))
                    if Player.Stats[stat]["Name"] == Player.Sand["Name"]:
                        Enemy.Accuracy["Value"] = Enemy.Accuracy["Max"] - Player.Sand["Value"]
                        #{Name}'s {Spell} rages on!
                        Scene.log.append(Scene.make_line(21, "Menu", {"{Name}": Player.Name["Value"], "{Spell}": Player.Sand["Name"]}))
                    if Player.Stats[stat]["Name"] == Player.Curse["Name"]:
                        ...
                        Scene.log.append(Scene.make_line(22, "Menu", {"{Name}": Enemy.Name["Value"], "{Curse}": Player.Curse["Name"]}))
                        if Player.Curse["Offering"] == True:
                            ...
                            if HP_and_MP == True:
                                ...
                                Scene.log.append(Scene.make_line(23, "Menu", 
                                                     {"{Name}": Player.Name["Value"], "{Amount}": [HP_amount, MP_amount], "{Stat}": [Player.HP["Name"], Player.MP["Name"]]}))
                            else:
                                #{Name} restored {Amount} {Stat}
                                Scene.log.append(Scene.make_line(4, "Menu", {"{Name}": Player.Name["Value"], "{Amount}": total_drained, "{Stat}": Player.MP["Name"]}))
                            if Player.HP["Value"] > Player.HP["Max"]:
                                Player.HP["Value"] = Player.HP["Max"]
...
#-----------------------------------------------------------------------------------------------------------------------------------
    ...
    def Drain_Condition(self, Player): #self, Class
        meets_condition = False
        if Player.Regen["Casts"] >= 5 and Player.Cure["Casts"] >= 20 and Player.Sacrifice["Value"] >= 100:
            meets_condition = True
        return meets_condition

    def Curse_Condition(self, Player): #self, Class
        meets_condition = False
        if Player.Drain["Casts"] >= 10 and Player.Regen["Casts"] >= 10 and Player.Sacrifice["Value"] >= 150:
            meets_condition = True
        return meets_condition
            ...
            if Player.Stats[stat]["Name"] == Spell:
                if Spell in conditions:
                    if conditions[Spell] == True:
                        ...
                        Scene.screen.addstr(Scene.make_line(19, "Menu", {"{Name}": Player.Name["Value"], "{Spell}": Player.Stats[stat]["Name"]}))
            ...
            if Player.Stats[stat]["Name"] == Spell:
                if "Duration" in Player.Stats[stat]:
                    ...
                    while loop == True:
                        ...
                        Scene.screen.addstr(Scene.line, 1, Scene.make_line(28, "Menu", {"{Stat}": [stats[0], stats[1]], "{Chapter}": Player.Upgrades["Name"]}))
                        #Re-draw Spellbook
                        for a in range(6, 15):
                            Scene.screen.addstr(a, 4, "                       ")
                            Scene.screen.addstr(a, 34, "                       ")
                        name_pos = 13 - int(len(Player.Stats[stat]["Name"])/2)
                        name_pos += 2
                        str_amount = 5
                        dur_amount = 1
                        Scene.screen.addstr(6, name_pos, Player.Stats[stat]["Name"], Scene.cyan)
                        Scene.screen.addstr(6, name_pos + 30, Player.Stats[stat]["Name"], Scene.cyan)
                        Scene.screen.addstr(9, 11, "Strength")
                        Scene.screen.addstr(10, 14, "+" + str(str_amount), Scene.cyan)
                        Scene.screen.addstr(9, 41, "Duration")
                        Scene.screen.addstr(10, 44, "+" + str(dur_amount), Scene.cyan)
                        response = Controller.get_response(Player, False, Scene)
                        response = response.title()
                        Player.view_spellbook(Scene, True)
                        if response == stats[0]: #Strength
                            Player.Stats[stat]["Value"] += str_amount
                            Scene.screen.addstr(Scene.line, 1, "* ", Scene.green)
                            Scene.screen.addstr(Scene.make_line(29, "Menu", {"{Spell}": Player.Stats[stat]["Name"]}))
                            loop = False
                        if response == stats[1]: #Duration
                            Player.Stats[stat]["Max"] += dur_amount
                            Scene.screen.addstr(Scene.line, 1, "* ", Scene.green)
                            Scene.screen.addstr(Scene.make_line(29, "Menu", {"{Spell}": Player.Stats[stat]["Name"]}))
                            loop = False
                else:
        ...
        for spell in self.exp_spells:
            if Player.known_spell(spell) == False:
                if spell == Player.Cure["Name"]:
                    ...
                    self.screen.addstr(Scene.print_message(20, False, "Menu", {}))
        Scene.Game_State.Story_UI(Player, Scene, Scene.Game_State.Controller)
